...
def draw_path(image, pos_history):
	for i in range(0, len(pos_history), 2):
		...
		img = cv2.circle(image, dot_pos, radius = 1, color = (0, 0, 0), thickness = -2)
...
def mean_shift(image, ker_size, min_change, start_pos, max_iters):
    ...
    while iters < max_iters:
        ...
        print(f"premik_x: {premik_x}, premik_y: {premik_y}")
...
density_map = generate_responses_1([70, 50], 1, [20, 70], 1.5, 15)
start_position = [70,50]
end_pos, iterations, pos_history = mean_shift(density_map, 9, 0.001, start_position, 10000)
...
print(f"Actual value: {density_map[70][20]}")

tracked_img = draw_path(density_map, pos_history)
